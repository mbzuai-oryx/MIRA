...
from llava.train.rag_handler import *
...
class DataArguments:
    ...
    rag_idx: str = field(default=None)
    rag_mdpath: str = field(default=None)
    rag_tokenizer: str = field(default=None)
    rag_topk: int = field(default=5)
    query_rewrite_enabled: bool = field(default=False)
    query_rewrite_host: str = field(default=None)
    query_rewrite_model: str = field(default=None)
...
def preprocess_llama_2(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, rag_seq: str = None) -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        conversations.append(conv.get_prompt(rag_seq))
...
def preprocess_gemma(sources: List[List[Dict[str, str]]], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, rag_seq: str = None) -> Dict:
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.", rag_seq: str = None) -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        if rag_seq is not None:
            final_system_message = system_message + "\n\n" + rag_seq
        input_id += tokenizer.apply_chat_template([{"role" : "system", "content" : final_system_message}])
...
def preprocess_v1(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, rag_seq: str = None) -> Dict:
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
def preprocess_mpt(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, rag_seq: str = None) -> Dict:
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, rag_seq: str = None) -> Dict:
    """
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    if conversation_lib.default_conversation.version == "mpt":
        return preprocess_mpt(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    if conversation_lib.default_conversation.version == "qwen":
        return preprocess_qwen(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    if conversation_lib.default_conversation.version == "gemma":
        return preprocess_gemma(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    if conversation_lib.default_conversation.version == "llama_v3":
        return preprocess_llama3(sources, tokenizer, has_image=has_image, rag_seq = rag_seq)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        self.use_rag = data_args.rag_enabled

        if self.use_rag:
            self.rag = query_rag_system(
                tokenizer = data_args.rag_tokenizer,
                index_path = data_args.rag_idx,
                metadata_path = data_args.rag_mdpath
            )
            self.query_rewrite = query_rewrite_handler(
                host_url = data_args.query_rewrite_host,
                target_model = data_args.query_rewrite_model
            )

            self.rag_top_k = data_args.rag_topk
            self.query_rewrite_enabled = data_args.query_rewrite_enabled
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # Generate RAG Dict
        if self.use_rag:
            rewritten_query = sources[-1][-1]["value"]
            
            nowidx = sources[-1]
            for cid in range(len(nowidx)-1, -1, -1):
                if nowidx[cid]["from"] == "human":
                    if self.query_rewrite_enabled:
                        rewritten_query = self.query_rewrite.rewrite(nowidx[:cid], nowidx[cid])
                    else:
                        rewritten_query = nowidx[cid]["value"]
                    break

            currentrag = self.rag.retrieve(rewritten_query, top_k = self.rag_top_k)

            ragdict_format = "These knowledge are for your reference:\n"
            for rr in currentrag:
                if not markdown_seq_check(rr["Content"]):
                    ragdict_format += rr['Paragraph'] + ":" + rr["Content"] + "\n"

            if ragdict_format == "These knowledge are for your reference:\n":
                ragdict_format = None

        has_image = ("image" in self.list_data_dict[i]) or ("video" in self.list_data_dict[i])
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image, rag_seq=ragdict_format if self.use_rag else None)
