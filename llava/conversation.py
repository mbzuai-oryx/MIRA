...
class Conversation:
    ...
    def get_prompt(self, rag_seq=None):
        # Add RAG sequence to system message if provided
        ...
        if self.sep_style == SeparatorStyle.SINGLE:
            ...
            ret = "" if system_message == "" else system_message + self.sep + "\n"
            for role, message in messages:
                if message:
                    if type(message) is tuple:
            ...
            chat_template_messages = [{"role": "system", "content": system_message}]
            for role, message in messages:
                if message:
                    if type(message) is tuple:
            ...
            for i, (role, message) in enumerate(messages):
                ...
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    if i == 0:
                        message = wrap_sys(system_message) + message
                    if i % 2 == 0:
